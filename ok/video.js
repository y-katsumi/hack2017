...
var GreyScale = function(){
  ...
  this.block_json = {};
  this.start = function(source_id, target_id, threshold){
    json = new Array();
    ...
    if (typeof s_obj.getContext === 'function') {
      ...
      t_ctx.drawImage(s_obj, 0, 0, s_obj.width, s_obj.height);
      var src = t_ctx.getImageData(0, 0, s_obj.width, s_obj.height);
      var dst = t_ctx.createImageData(s_obj.width, s_obj.height);
    }
...
console.log(this.rego);
    ...
    block_json = this.separate(json, threshold);
  ...
  // ブロックにわける
  this.separate = function(json, threshold){
    ...
    function getMedian(json, i_start, j_start, i_add, j_add){
      block = {};
      median = [];
      i = i_start + parseInt(i_add / 2);
      j = j_start + parseInt(j_add / 2);
      // r = parseInt(
      //   (json[i - 1][j - 1].r + json[i - 1][j].r + json[i - 1][j + 1].r + json[i][j - 1].r + json[i][j].r + json[i][j + 1].r + json[i + 1][j - 1].r + json[i + 1][j].r + json[i + 1][j + 1].r) / 9);
      // g = parseInt(
      //   (json[i - 1][j - 1].g + json[i - 1][j].g + json[i - 1][j + 1].g + json[i][j - 1].g + json[i][j].g + json[i][j + 1].g + json[i + 1][j - 1].g + json[i + 1][j].g + json[i + 1][j + 1].g) / 9);
      // b = parseInt(
      //   (json[i - 1][j - 1].b + json[i - 1][j].b + json[i - 1][j + 1].b + json[i][j - 1].b + json[i][j].b + json[i][j + 1].b + json[i + 1][j - 1].b + json[i + 1][j].b + json[i + 1][j + 1].b) / 9);
      r = json[i][j].r;
      g = json[i][j].g;
      b = json[i][j].b;
      block = {
